...
dns_out_host() {
	...
	echo
        echo '	Adcione a diretiva "allow-update" com o parâmetro "{ key NOME_DA_KEY_RNDC; };" na zona em "/etc/bind/named.conf.local".'
	echo '	Adcione uma diretiva "key" utilizando o "cat /etc/bind/key-rndc >> /etc/bind/named.conf.options" para usar uma chave já existente,'
        echo '		ou crie uma nova com "rndc-confgen -a -b 51".'
	echo '	Adcione uma diretiva "controls" para definir quem poderá fazer as configurações remotas.'
	...
	echo '	Adcione a mesma diretiva "key" antes feita no "/etc/bind/named.conf.options" do DNS.'
	echo '	Adcione uma diretiva "zone" referente a zona em /etc/bind/named.conf.local no DNS.'
	echo ' 	Reinicie o isc-dhcpd.'
	echo
	while true; do
